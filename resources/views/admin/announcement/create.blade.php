...
<div class="space-y-6 max-w-2xl">
    <div>
        ...
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white mt-2">Buat Pengumuman Baru</h1>
    </div>
        ...
        <form method="POST" action="{{ route('announcement.store') }}" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" id="title" name="title" value="{{ old('title') }}"
                       placeholder="Contoh: Libur Nasional — Semua Kendaraan Harus Dikembalikan"
            ...
            <div>
                ...
                <textarea id="content" name="content" rows="5"
                          ...
                          class="input-base mt-1" required>{{ old('content') }}</textarea>
            </div>
            <div class="flex items-center gap-3 p-3 bg-gray-50 dark:bg-slate-700/30 rounded-xl">
                ...
                <label for="is_active" class="text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer">
                    Aktifkan pengumuman ini sekarang (langsung tampil di dashboard pengguna)
                </label>
            </div>
            <div class="flex items-center justify-end gap-3 pt-2">
                <a href="{{ route('announcement.index') }}" class="btn-secondary">Batal</a>
                <button type="submit" class="btn-primary">Terbitkan Pengumuman</button>
            </div>
        </form>
    </div>
</div>
